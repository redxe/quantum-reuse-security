...
def validate_quantum_computation(
    ...
    """Validate computed branch density matrices and aggregate error metrics."""
    validation_report = {
...
def bob_probabilities(value: int, basis: int, bob_basis: int) -> tuple[float, float]:
    """Compute Bob's exact q2 measurement probabilities for a chosen basis."""
...
def detector_accepts(value: int, basis: int, bob_basis: int, bob_result: int) -> bool:
    """Detector acceptance event for the reconstructed educational circuit."""
    return (bob_basis != basis) or (bob_result == value)


def detector_acceptance_probability(value: int, basis: int, bob_basis: int) -> float:
    """Acceptance probability induced by detector_accepts over Bob outcomes."""
...
def branch_trace_bob_result_probabilities(
    ...
    """Reconstruct Bob q2 outcome probabilities from explicit branch events."""
    n = 5
...
def detector_acceptance_probability_from_branch_trace(
    ...
    """Compute detector acceptance using explicit branch-trace composition."""
    total = 0.0
...
def average_fifth_state(value: int, basis: int, eve_basis: int) -> np.ndarray:
    """Average fifth-wire reduced state over Eve measurement outcomes."""
...
def run_analysis(output_dir: Path) -> dict:
    ...
    if validation_report["all_warnings"]:
        print("WARNINGS:")
        for warning in validation_report["all_warnings"][:10]:
            print(f"  - {warning}")
    ...
    if validation_report["max_condition_number"] < np.inf:
        ...
        print("  (errors above this may lose meaning)")
    ...
    validation_summary = canonicalize_validation_summary(
        {
            ...
            "error_bounds": {
                "spectral_perturbation_bound": error_bounds[
                    "spectral_perturbation_bound"
                ],
